...
public class License {
    ...
    private String multilineValueString(String s) {
        List<String> lines = new ArrayList<String>(Arrays.asList(s.split("\n")));
        final StringBuilder sb = new StringBuilder();
        int i = 0;
    ...
    public UUID fingerprint() {
        try {
            final ByteBuffer bb = ByteBuffer.wrap(MessageDigest.getInstance("MD5").digest(
                serialized(Collections.unmodifiableSet(
                        new HashSet<String>(Arrays.asList(SIGNATURE_KEY, DIGEST_KEY))))));
